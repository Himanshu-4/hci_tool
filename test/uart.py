...
class AsyncSerialPort:
    def __init__(self, port_url, baudrate, loop=None, read_buffer_size=1024, **kwargs):
    ...
    async def connect(self):
        async with self._connection_lock:
            if self.is_connected:
                print(f"Already connected to {self.port_url}")
                return True
            
            print(f"Attempting to connect to {self.port_url} at {self.baudrate} baud...")
            try:
                self._reader, self._writer = await serial_asyncio.open_serial_connection(
                    url=self.port_url,
                    baudrate=self.baudrate,
                    loop=self._loop, # Though loop is often implicitly handled by asyncio context
                    **self.serial_kwargs
                )
                ...
                print(f"Successfully connected to {self.port_url}")
                return True
            except SerialException as e:
                print(f"Failed to connect to {self.port_url}: {e}")
                self.is_connected = False
                return False
            except Exception as e:
                print(f"An unexpected error occurred during connection to {self.port_url}: {e}")
                self.is_connected = False
                return False

    async def disconnect(self):
        async with self._connection_lock:
            if not self.is_connected and not (self._read_task or self._write_task): # Check if already disconnected
                print(f"{self.port_url} is already disconnected.")
                return

            print(f"Disconnecting from {self.port_url}...")
            self._disconnect_requested.set() # Signal loops to stop

            if self._read_task:
                try:
                    # Give a bit of time for the loop to see the event
                    await asyncio.wait_for(self._read_task, timeout=1.0) 
                except asyncio.TimeoutError:
                    self._read_task.cancel()
                except Exception as e:
                    print(f"Error during read_task cleanup for {self.port_url}: {e}")
                finally:
                    self._read_task = None
            
            if self._write_task:
                try:
                    # Ensure write queue is processed or task finishes
                    await self.write_queue.put(None) # Sentinel value to stop writer
                    await asyncio.wait_for(self._write_task, timeout=2.0)
                except asyncio.TimeoutError:
                     self._write_task.cancel()
                except Exception as e:
                    print(f"Error during write_task cleanup for {self.port_url}: {e}")
                finally:
                    self._write_task = None
            ...
            if self._writer:
                try:
                    ...
                    print(f"Error closing writer for {self.port_url}: {e}")
                finally:
                    self._writer = None
            
            self._reader = None # Reader is managed by StreamWriter closure in pyserial-asyncio typically
            self.is_connected = False
            print(f"Disconnected from {self.port_url}")

    async def _read_loop(self):
        print(f"Read loop started for {self.port_url}")
        try:
            while not self._disconnect_requested.is_set() and self._reader:
                try:
                    ...
                    if data:
                        ...
                        # print(f"[{self.port_url}] RX: {data}") # For debugging
                    else: # EOF, port likely closed by other end
                        print(f"Read EOF from {self.port_url}, port may have closed.")
                        self._disconnect_requested.set()
                        break
                except asyncio.TimeoutError:
                    continue # No data, just continue polling
                except SerialException as e:
                    print(f"SerialException in read loop for {self.port_url}: {e}")
                    self._disconnect_requested.set()
                    break
                except asyncio.CancelledError:
                    print(f"Read loop cancelled for {self.port_url}")
                    break
                except Exception as e:
                    print(f"Unexpected error in read loop for {self.port_url}: {e}")
                    self._disconnect_requested.set()
                    break
        finally:
            print(f"Read loop stopped for {self.port_url}")
            if self._disconnect_requested.is_set() and self.is_connected: # If disconnect was triggered by error
                self._loop.create_task(self.disconnect()) # Ensure full disconnect procedure

    async def _write_loop(self):
        print(f"Write loop started for {self.port_url}")
        try:
            while not self._disconnect_requested.is_set() or not self.write_queue.empty():
                try:
                    ...
                    if data_to_write is None and self._disconnect_requested.is_set(): # Sentinel check
                        self.write_queue.task_done()
                        break
                    if data_to_write and self._writer:
                        # print(f"[{self.port_url}] TX: {data_to_write}") # For debugging
                        self._writer.write(data_to_write)
                        await self._writer.drain()
                        self.write_queue.task_done()
                    elif not self._writer and data_to_write: # If writer closed but data remains
                        print(f"Writer for {self.port_url} is closed, cannot send data: {data_to_write}")
                        self.write_queue.task_done() # Acknowledge item
                    ...
                    continue # No data, check disconnect flag and loop again
                except SerialException as e:
                    print(f"SerialException in write loop for {self.port_url}: {e}")
                    self._disconnect_requested.set()
                    break # Stop writing on serial error
                except asyncio.CancelledError:
                    print(f"Write loop cancelled for {self.port_url}")
                    break
                except Exception as e:
                    print(f"Unexpected error in write loop for {self.port_url}: {e}")
                    self._disconnect_requested.set()
                    break # Stop on other errors
        finally:
            print(f"Write loop stopped for {self.port_url}")
            # Clear any remaining items in the queue if disconnect was abrupt
            while not self.write_queue.empty():
                try:
                    item = self.write_queue.get_nowait()
                    print(f"Discarding unsent item from {self.port_url}: {item}")
                    self.write_queue.task_done()
    ...
    async def read_data(self, timeout=None):
        if not self.is_connected and self.read_queue.empty():
            # print(f"Not connected and no data in queue for {self.port_url}")
            return None
        try:
            ...
            print(f"Error reading from queue for {self.port_url}: {e}")
            return None

    async def write_data(self, data):
        if not self.is_connected and not self._writer : # Also check if writer is available
             print(f"Cannot write: Not connected to {self.port_url} or writer not available.")
             return False
        try:
            await self.write_queue.put(data)
            return True
        except Exception as e:
            print(f"Error putting data to write queue for {self.port_url}: {e}")
            return False
...
class SerialManager:
    ...
    def add_port(self, port_id, port_url, baudrate, read_buffer_size=1024, **kwargs):
        if port_id in self.serial_ports:
            print(f"Port ID {port_id} already exists.")
            return self.serial_ports[port_id]
        
        port = AsyncSerialPort(port_url, baudrate, loop=self._loop, read_buffer_size=read_buffer_size, **kwargs)
        self.serial_ports[port_id] = port
        print(f"Added port {port_id} for {port_url}")
        return port

    async def connect_port(self, port_id):
        if port_id not in self.serial_ports:
            print(f"Port ID {port_id} not found.")
            return False
        return await self.serial_ports[port_id].connect()

    async def disconnect_port(self, port_id):
        if port_id not in self.serial_ports:
            print(f"Port ID {port_id} not found.")
            return False
        await self.serial_ports[port_id].disconnect()
        return True

    async def connect_all(self):
        results = await asyncio.gather(*(port.connect() for port in self.serial_ports.values()), return_exceptions=True)
        for port, result in zip(self.serial_ports.values(), results):
            if isinstance(result, Exception):
                print(f"Error connecting {port.port_url}: {result}")
            elif not result:
                 print(f"Failed to connect {port.port_url} (no exception, but connect returned False)")
        return all(res is True for res in results if not isinstance(res, Exception))


    async def disconnect_all(self):
        await asyncio.gather(*(port.disconnect() for port in self.serial_ports.values()), return_exceptions=True)
        print("Disconnect all command issued.")

    def get_port(self, port_id) -> AsyncSerialPort | None:
        return self.serial_ports.get(port_id)

    def get_all_statuses(self):
        return {port_id: port.get_status() for port_id, port in self.serial_ports.items()}

# --- Example Usage ---
async def main():
    # On Linux, you might use '/dev/ttyUSB0', '/dev/ttyUSB1'
    ...
    # Then use /tmp/vsp1_tx and /tmp/vsp2_tx as port_url in the code,
    # and connect to /tmp/vsp1_rx and /tmp/vsp2_rx with a serial terminal program.

    port_configs = {
        "device1": {"url": "/tmp/vsp1_tx", "baudrate": 9600},
        "device2": {"url": "/tmp/vsp2_tx", "baudrate": 115200, "timeout": 0.1} # Example of additional pyserial kwarg
    }
    
    manager = SerialManager()

    for port_id, config in port_configs.items():
        manager.add_port(port_id, config["url"], config["baudrate"], **config.get("kwargs", {}))

    if not await manager.connect_all():
        print("Failed to connect one or more ports. Exiting.")
        # Optionally attempt to disconnect those that did connect
        await manager.disconnect_all()
        return

    print("\nAll specified ports connected (or attempted).")
    print("Initial statuses:", manager.get_all_statuses())

    # Example: Send data to device1
    dev1 = manager.get_port("device1")
    if dev1 and dev1.is_connected:
        await dev1.write_data(b"Hello from Device1!\n")

    # Example: Send data to device2
    dev2 = manager.get_port("device2")
    if dev2 and dev2.is_connected:
        await dev2.write_data(b"Test data for Device2\n")

    # Listen for incoming data for a while
    print("\nListening for incoming data for 10 seconds...")
    for _ in range(20): # Check every 0.5 seconds for 10 seconds
        for port_id, port_obj in manager.serial_ports.items():
            if port_obj.is_connected:
                received = await port_obj.read_data(timeout=0.01) # Short timeout for non-blocking check
                if received:
                    print(f"[{port_id} RX ({port_obj.port_url})]: {received.decode(errors='replace')}", end='')
        await asyncio.sleep(0.5)
    
    print("\nFinished listening.")
    print("Final statuses:", manager.get_all_statuses())
...
if __name__ == "__main__":
    try:
        ...
        # Ensure event loop closes cleanly if it was started by asyncio.run()
        # Additional cleanup can be done here if necessary outside the main async context,
        # but typically asyncio.run() handles loop closure.
